...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".airabot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.airabot${path.sep}media`);
