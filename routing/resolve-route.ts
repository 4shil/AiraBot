...
function listAgents(cfg: AiraBotConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AiraBotConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AiraBotConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AiraBotConfig, EvaluatedBindingsCache>();
