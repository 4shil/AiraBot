...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `airabot-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        airabot: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      airabot: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.airabot/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without airabot.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@airabot/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("airabot.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@airabot/voice-call@0.0.1", "--ignore-scripts"]);
