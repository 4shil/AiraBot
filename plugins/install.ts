...
async function ensureAiraBotExtensions(manifest: PackageManifest) {
  const extensions = manifest[MANIFEST_KEY]?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing airabot.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json airabot.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureAiraBotExtensions(manifest);
  } catch (err) {
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "airabot security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "airabot security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(params: {
  ...
  return await withTempDir("airabot-plugin-", async (tmpDir) => {
...
export async function installPluginFromNpmSpec(params: {
  ...
  return await withTempDir("airabot-npm-pack-", async (tmpDir) => {
