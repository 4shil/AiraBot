...
function createSinglePluginWithEmptySlotsConfig(): AiraBotConfig {
  return {
    plugins: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "airabot-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-airabot-extensions/my-plugin",
      },
      extensionsDir,
    });
