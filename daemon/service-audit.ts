...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.AIRABOT_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service AIRABOT_GATEWAY_TOKEN does not match gateway.auth.token in airabot.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `airabot gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
