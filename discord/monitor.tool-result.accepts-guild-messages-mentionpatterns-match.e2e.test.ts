  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/airabot",
      },
    },
    session: { store: "/tmp/airabot-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bairabot\\b"] },
        },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "AiraBot" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/airabot" },
      session: { store: "/tmp/airabot-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
