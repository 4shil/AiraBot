...
import type { AiraBotConfig } from "../config/types.js";
import { resolvePreferredAiraBotTmpDir } from "../infra/tmp-airabot-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredAiraBotTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "airabot.log"); // legacy single-file path

const LOG_PREFIX = "airabot";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AiraBotConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => AiraBotConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.AIRABOT_TEST_FILE_LOG !== "1" ? "silent" : "info";
