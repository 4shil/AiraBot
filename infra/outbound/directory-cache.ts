...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AiraBotConfig | null = null;
  ...
  get(key: string, cfg: AiraBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AiraBotConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AiraBotConfig): void {
