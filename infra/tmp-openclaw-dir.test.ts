...
import { POSIX_OPENCLAW_TMP_DIR, resolvePreferredAiraBotTmpDir } from "./tmp-airabot-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAiraBotTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `airabot-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAiraBotTmpDir({
    accessSync,
...
describe("resolvePreferredAiraBotTmpDir", () => {
  it("prefers /tmp/airabot when it already exists and is writable", () => {
  ...
  it("prefers /tmp/airabot when it does not exist but /tmp is writable", () => {
  ...
  it("falls back to os.tmpdir()/airabot when /tmp/airabot is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/airabot when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/airabot is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/airabot is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/airabot is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
