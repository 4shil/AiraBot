...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".airabot", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(base, ".airabot");
  ...
  it("strips embedded CR/LF from stored auth profiles (token + api_key)", async () => {
    await withSuiteHome(
      async (home) => {
        const agentDir = path.join(home, ".airabot", "agents", "main", "agent");
