...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__airabot_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "airabot-root");
...
describe("resolveAiraBotPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveAiraBotPackageRootSync } = await import("./airabot-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "airabot");
    const pkgRoot = path.join(project, "node_modules", "airabot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "airabot" }));

    expect(resolveAiraBotPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "airabot");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "airabot.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "airabot" }));

    expect(resolveAiraBotPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAiraBotPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-airabot package roots", async () => {
    const { resolveAiraBotPackageRootSync } = await import("./airabot-root.js");

    const pkgRoot = fx("not-airabot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-airabot" }));

    expect(resolveAiraBotPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveAiraBotPackageRoot } = await import("./airabot-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "airabot" }));

    await expect(resolveAiraBotPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
