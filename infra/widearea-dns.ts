...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AIRABOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*airabot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "airabot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "airabot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "airabot-gw");
  ...
  records.push(`_airabot-gw._tcp IN PTR ${instanceLabel}._airabot-gw._tcp`);
  records.push(`${instanceLabel}._airabot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._airabot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; airabot-content-hash: ${contentHash}\n${contentBody}`;
}
