...
function candidateBinDirs(opts: EnsureAiraBotPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `airabot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "airabot");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AIRABOT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "airabot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `airabot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAiraBotCliOnPath(opts: EnsureAiraBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AIRABOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.AIRABOT_PATH_BOOTSTRAPPED = "1";
