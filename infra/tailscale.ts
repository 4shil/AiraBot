...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.AIRABOT_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for AiraBot. You can keep running the web gateway without it: \`${formatCliCommand("airabot gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
