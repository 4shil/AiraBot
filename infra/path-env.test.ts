...
let ensureAiraBotCliOnPath: typeof import("./path-env.js").ensureAiraBotCliOnPath;

describe("ensureAiraBotCliOnPath", () => {
  const envKeys = [
    "PATH",
  ...
  beforeAll(async () => {
    ({ ensureAiraBotCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling airabot exists", () => {
    const tmp = abs("/tmp/airabot-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "airabot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/airabot-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "airabot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/airabot-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "airabot");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "airabot");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/airabot-path/case-linuxbrew");
