...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-update-"));
  });
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "airabot.mjs"), "export {};\n", "utf-8");
  });
  ...
  function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "airabot.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "airabot", version: "1.0.0" };
  ...
  async function seedGlobalPackageRoot(pkgRoot: string, version = "1.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "airabot", version }),
      "utf-8",
    );
  }
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "airabot.mjs")} doctor --non-interactive --fix`]:
        {
          stdout: "",
        },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "airabot", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const pkgRoot = path.join(nodeModules, "airabot");
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "airabot", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g airabot@latest",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g airabot@beta",
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "airabot");
    const staleDir = path.join(nodeModules, ".airabot-stale");
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "airabot");
      await seedGlobalPackageRoot(pkgRoot);

      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g airabot@latest",
        onInstall: async () => {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "airabot", version: "2.0.0" }),
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g airabot@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a airabot checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-airabot-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when airabot.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "airabot.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("airabot doctor entry");
  });
