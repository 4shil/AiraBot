...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("airabot-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("airabot (AiraBot)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("airabot");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("airabot.local");
