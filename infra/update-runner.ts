...
const DEFAULT_PACKAGE_NAME = "airabot";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "airabot.mjs");
