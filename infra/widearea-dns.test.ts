...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "airabot.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (AiraBot)",
    ...
    expect(txt).toContain(`$ORIGIN airabot.internal.`);
    ...
    expect(txt).toContain(`_airabot-gw._tcp IN PTR studio-london._airabot-gw._tcp`);
    expect(txt).toContain(`studio-london._airabot-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (AiraBot)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/airabot`);
  });
