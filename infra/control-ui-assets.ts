...
import { resolveAiraBotPackageRoot, resolveAiraBotPackageRootSync } from "./airabot-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAiraBotPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "airabot" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAiraBotPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
