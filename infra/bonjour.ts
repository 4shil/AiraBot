...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AIRABOT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "AiraBot";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(AiraBot\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.AIRABOT_MDNS_HOSTNAME?.trim() ||
