...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("airabot-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-airabot-archive.tgz");
  ...
  it("rejects unsupported archive extensions", async () => {
    const dir = await createTempDir("airabot-install-source-utils-");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using the final non-empty stdout line", async () => {
    const cwd = await createTempDir("airabot-install-source-utils-");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: "npm notice created package\nairabot-plugin-1.2.3.tgz\n",
      stderr: "",
      code: 0,
      signal: null,
      killed: false,
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "airabot-plugin-1.2.3.tgz"),
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "airabot-plugin@1.2.3", "--ignore-scripts"],
