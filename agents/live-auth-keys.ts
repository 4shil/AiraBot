...
const GOOGLE_LIVE_SINGLE_KEY = "AIRABOT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "AIRABOT_LIVE_ANTHROPIC_KEY",
    listVar: "AIRABOT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "AIRABOT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `AIRABOT_LIVE_${base}_KEY`;
