...
function resolveSkillsLimits(config?: AiraBotConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "airabot-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "airabot-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAiraBotMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AiraBotConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`airabot skills check\` to audit.`
    : "";
