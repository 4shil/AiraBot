...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "airabot-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "airabot-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "airabot.sandbox=1",
        "--label",
        "airabot.sessionKey=main",
        "--label",
        "airabot.createdAtMs=1700000000000",
        "--label",
        "airabot.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
  ...
  it("throws on dangerous bind mounts (parent path)", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "airabot-sbx-dangerous-parent",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
      }),
