...
import { createAiraBotCodingTools } from "../pi-tools.js";
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".airabot-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): AiraBotConfig {
  return {
    agents: {
      defaults: {
...
describe("image tool implicit imageModel config", () => {
  ...
  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-image-"));
    const cfg: AiraBotConfig = {
  ...
  it("allows workspace images via createAiraBotCodingTools default workspace root", async () => {
    await withTempWorkspacePng(async ({ imagePath }) => {
      ...
      try {
        ...
        const tools = createAiraBotCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-image-sandbox-"));
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    ...
    const res = await tool.execute("t1", {
      prompt: "Describe the image.",
      image: "@/Users/steipete/.airabot/media/inbound/photo.png",
    });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: AiraBotConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.1" } } },
    };
