  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "airabot.configHash"')
      ) {
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`airabot.configHash=${newHash}`);
