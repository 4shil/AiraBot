...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "airabot.sandbox=1");
  args.push("--label", `airabot.sessionKey=${params.scopeKey}`);
  args.push("--label", `airabot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `airabot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "airabot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`airabot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`airabot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("airabot sandbox recreate --all");
}
