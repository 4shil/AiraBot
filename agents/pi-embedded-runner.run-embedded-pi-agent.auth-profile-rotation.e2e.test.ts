...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): AiraBotConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-workspace-"));
    const now = Date.now();
