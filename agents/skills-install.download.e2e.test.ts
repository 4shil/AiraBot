...
const originalAiraBotStateDir = process.env.OPENCLAW_STATE_DIR;

afterEach(() => {
  if (originalAiraBotStateDir === undefined) {
    delete process.env.OPENCLAW_STATE_DIR;
  } else {
    process.env.OPENCLAW_STATE_DIR = originalAiraBotStateDir;
  }
});
...
describe("installSkill download extraction safety", () => {
  ...
  it("rejects zip slip traversal", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-skills-install-"));
    try {
