...
function makeCfg(overrides: Partial<AiraBotConfig> = {}): AiraBotConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeFallbacksOnlyCfg(): AiraBotConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AiraBotConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-auth-"));
