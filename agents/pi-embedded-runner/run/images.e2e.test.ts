...
[media attached 1/2: /Users/tyleryust/.airabot/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.airabot/media/IMG_6431.jpeg (image/jpeg)]
...
what is this?`;
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const prompt = `[media attached: /Users/test/.airabot/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`;
    const refs = detectImageReferences(prompt);
...
describe("loadImageFromRef", () => {
  it("allows sandbox-validated host paths outside default media roots", async () => {
    const sandboxParent = await fs.mkdtemp(path.join(os.homedir(), "airabot-sandbox-image-"));
    try {
