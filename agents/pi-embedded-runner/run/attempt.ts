...
import { resolveAiraBotAgentDir } from "../../agent-paths.js";
...
import { resolveAiraBotDocsPath } from "../../docs-path.js";
...
import { createAiraBotCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveAiraBotAgentDir();

    // Check if the model supports native image input
    const modelHasVision = params.model.input?.includes("image") ?? false;
    const toolsRaw = params.disableTools
      ? []
      : createAiraBotCodingTools({
          exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveAiraBotDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/airabot/airabot/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt") {
          try {
            sessionManager.appendCustomEntry("airabot:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/airabot/airabot/issues/8643
