...
function resolveExecConfig(params: { cfg?: AiraBotConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: AiraBotConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createAiraBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAiraBotReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
