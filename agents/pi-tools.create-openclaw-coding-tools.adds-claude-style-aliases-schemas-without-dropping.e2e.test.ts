...
import { createAiraBotTools } from "./airabot-tools.js";
import { __testing, createAiraBotCodingTools } from "./pi-tools.js";
import { createAiraBotReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createAiraBotCodingTools();
...
describe("createAiraBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAiraBotCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createAiraBotCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "airabot-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createAiraBotReadTool(
      baseRead as unknown as Parameters<typeof createAiraBotReadTool>[0],
    );
