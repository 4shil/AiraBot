...
describe("createAiraBotCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    if (process.platform === "win32") {
      return;
    }

    const { createAiraBotCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airabot-safe-bins-"));
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    ...
    const { createAiraBotCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airabot-safe-bins-expand-"));
