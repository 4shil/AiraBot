...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.AIRABOT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".airabot", `workspace-${profile}`);
  }
  return path.join(home, ".airabot", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".airabot";
