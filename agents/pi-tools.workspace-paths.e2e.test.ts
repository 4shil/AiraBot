...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("airabot-ws-", async (workspaceDir) => {
      await withTempDir("airabot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAiraBotCodingTools({ workspaceDir });
    ...
    await withTempDir("airabot-ws-", async (workspaceDir) => {
      await withTempDir("airabot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello airabot");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("airabot-ws-", async (workspaceDir) => {
      await withTempDir("airabot-override-", async (overrideDir) => {
        const tools = createAiraBotCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("airabot-sandbox-", async (sandboxDir) => {
      await withTempDir("airabot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "airabot-sbx-test",
          containerWorkdir: "/workspace",
          fsBridge: createHostSandboxFsBridge(sandboxDir),
          docker: {
            image: "airabot-sandbox:bookworm-slim",
            containerPrefix: "airabot-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createAiraBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
