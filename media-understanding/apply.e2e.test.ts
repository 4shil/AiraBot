...
function createGroqAudioConfig(): AiraBotConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AiraBotConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function createTempMediaFile(params: { fileName: string; content: Buffer | string }) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-media-"));
