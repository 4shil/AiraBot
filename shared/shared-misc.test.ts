...
describe("shared/frontmatter", () => {
  ...
  test("resolveAiraBotManifestBlock parses JSON5 metadata and picks airabot block", () => {
    const frontmatter = {
      metadata: "{ airabot: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAiraBotManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveAiraBotManifestBlock returns undefined for invalid input", () => {
    expect(resolveAiraBotManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveAiraBotManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveAiraBotManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
