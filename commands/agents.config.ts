...
export function listAgentEntries(cfg: AiraBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AiraBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: AiraBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AiraBotConfig): AgentSummary[] {
