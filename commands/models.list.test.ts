...
const ensureAiraBotModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/airabot-agent/auth.json" });
const resolveAiraBotAgentDir = vi.fn().mockReturnValue("/tmp/airabot-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/airabot-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureAiraBotModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/airabot-agent");
  });
