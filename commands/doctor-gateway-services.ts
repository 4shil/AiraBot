...
function resolveGatewayAuthToken(cfg: AiraBotConfig, env: NodeJS.ProcessEnv): string | undefined {
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing AiraBot gateway next.");
      }
    }
  }
