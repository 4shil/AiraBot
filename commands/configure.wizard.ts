...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.AIRABOT_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.AIRABOT_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.airabot.ai/gateway/health",
        "https://docs.airabot.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from airabot.json",
        },
      ],
...
async function promptWebToolsConfig(
  nextConfig: AiraBotConfig,
  runtime: RuntimeEnv,
): Promise<AiraBotConfig> {
  ...
  note(
    [
      ...
      "Docs: https://docs.airabot.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "AiraBot update wizard" : "AiraBot configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: AiraBotConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.airabot.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("airabot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.AIRABOT_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.AIRABOT_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.AIRABOT_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.AIRABOT_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.AIRABOT_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.airabot.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
