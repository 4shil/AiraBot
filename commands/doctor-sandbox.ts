...
function resolveSandboxDockerImage(cfg: AiraBotConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: AiraBotConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: AiraBotConfig, image: string): AiraBotConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: AiraBotConfig, image: string): AiraBotConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export function noteSandboxScopeWarnings(cfg: AiraBotConfig) {
