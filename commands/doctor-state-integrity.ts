...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".airabot"].map((dir) => path.resolve(root, entry.name, dir));
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".airabot");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.airabot out of git; it contains credentials and session history.",
