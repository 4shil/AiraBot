...
import type { AiraBotConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: AiraBotConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.AIRABOT_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.AIRABOT_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: AiraBotConfig) {
