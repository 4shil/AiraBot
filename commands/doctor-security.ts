...
import type { AiraBotConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { readChannelAllowFromStore } from "../pairing/pairing-store.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: AiraBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("airabot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("airabot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("airabot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("airabot doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "airabot config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("airabot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('airabot config set session.dmScope "per-channel-peer"') +
