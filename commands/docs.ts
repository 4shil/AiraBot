...
const SEARCH_TOOL = "https://docs.airabot.ai/mcp.SearchAiraBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.airabot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('airabot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.airabot.ai/");
      runtime.log(`Search: ${formatCliCommand('airabot docs "your query"')}`);
    }
    return;
  }
