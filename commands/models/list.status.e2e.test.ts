...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAiraBotAgentDir: vi.fn().mockReturnValue("/tmp/airabot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/airabot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/airabot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAiraBotAgentDir: mocks.resolveAiraBotAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/airabot-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAiraBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/airabot-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/airabot-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/airabot-agent-custom");
