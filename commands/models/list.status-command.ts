...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAiraBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("airabot models auth setup-token")}\` or \`${formatCliCommand("airabot configure")}\`.`
          : `Run \`${formatCliCommand("airabot configure")}\` or set an API key env var.`;
