...
export async function updateConfig(
  mutator: (cfg: AiraBotConfig) => AiraBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AiraBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AiraBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("airabot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
