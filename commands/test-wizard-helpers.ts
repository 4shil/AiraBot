...
export async function setupAuthTestEnv(
  ...
  process.env.AIRABOT_STATE_DIR = stateDir;
  process.env.AIRABOT_AGENT_DIR = agentDir;
...
export function requireAiraBotAgentDir(): string {
  const agentDir = process.env.AIRABOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AIRABOT_AGENT_DIR not set");
  }
  return agentDir;
}
