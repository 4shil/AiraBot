...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.AIRABOT_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: AiraBotConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.AIRABOT_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
