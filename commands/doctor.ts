...
function resolveMode(cfg: AiraBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AiraBot doctor");

  const root = await resolveAiraBotPackageRoot({
  ...
  let cfg: AiraBotConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("airabot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("airabot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("airabot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("airabot doctor --fix")}" to apply changes.`);
  }
