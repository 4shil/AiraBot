 ...
 * Runs as part of `airabot doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: AiraBotConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("airabot config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("airabot memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`airabot auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("airabot config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("airabot memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("airabot auth add --provider openai")}`,
