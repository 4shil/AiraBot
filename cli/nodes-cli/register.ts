    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["airabot nodes status", "List known nodes with live status."],
          ["airabot nodes pairing pending", "Show pending node pairing requests."],
          ['airabot nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["airabot nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.airabot.ai/cli/nodes")}\n`,
    );
