...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("airabot node install"), formatCliCommand("airabot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("airabot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AIRABOT_LOG_PREFIX: baseEnv.AIRABOT_LOG_PREFIX ?? "node",
