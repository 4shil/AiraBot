    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "airabot node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["airabot node status", "Check node host service status."],
          ["airabot node install", "Install the node host service."],
          ["airabot node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.airabot.ai/cli/node")}\n`,
    );
