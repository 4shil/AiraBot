...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.airabot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AiraBot hooks")
    ...
    .option("--hook-url <url>", "AiraBot hook URL")
    .option("--hook-token <token>", "AiraBot hook token")
