...
import { resolveAiraBotPackageRoot } from "../../infra/airabot-root.js";
...
const AIRABOT_REPO_URL = "https://github.com/airabot/airabot.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "airabot";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("airabot@")) {
    return trimmed.slice("airabot@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.AIRABOT_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveAiraBotPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AIRABOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AIRABOT_GIT_DIR points at a non-git directory: ${params.dir}. Set AIRABOT_GIT_DIR to an empty folder or an airabot checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AIRABOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "airabot.mjs");
