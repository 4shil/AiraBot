...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.AIRABOT_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.AIRABOT_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.AIRABOT_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.AIRABOT_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.AIRABOT_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.AIRABOT_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `airabot-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `airabot-restart-${timestamp}.bat`;
