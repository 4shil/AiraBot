...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AiraBot Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(airabot completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# AiraBot Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(airabot completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $AIRABOT_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "airabot") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c airabot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'airabot gateway' and not 'airabot other gateway', we need to check the sequence.
