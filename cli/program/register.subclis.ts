...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.AIRABOT_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.AIRABOT_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<AiraBotConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live AiraBot docs",
    hasSubcommands: false,
    register: async (program) => {
