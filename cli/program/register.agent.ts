...
${formatHelpExamples([
  ['airabot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['airabot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'airabot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'airabot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['airabot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'airabot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.airabot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.airabot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['airabot agents set-identity --agent main --name "AiraBot" --emoji "🦞"', "Set name + emoji."],
  ["airabot agents set-identity --agent main --avatar avatars/airabot.png", "Set avatar path."],
  [
    "airabot agents set-identity --workspace ~/.airabot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "airabot agents set-identity --identity-file ~/.airabot/workspace/IDENTITY.md --agent main",
