...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AiraBot and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["airabot update", "Update a source checkout (git)"],
        ["airabot update --channel beta", "Switch to beta channel (git + npm)"],
        ["airabot update --channel dev", "Switch to dev channel (git + npm)"],
        ["airabot update --tag beta", "One-off update to a dist-tag or version"],
        ["airabot update --no-restart", "Update without restarting the service"],
        ["airabot update --json", "Output result as JSON"],
        ["airabot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["airabot update wizard", "Interactive update wizard"],
        ["airabot --update", "Shorthand for airabot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run airabot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.airabot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["airabot update status", "Show channel + version status."],
          ["airabot update status --json", "JSON output."],
          ["airabot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.airabot.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
