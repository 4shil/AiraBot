...
import type { ConfigFileSnapshot, AiraBotConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `airabot config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: AiraBotConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: AiraBotConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: AiraBotConfig, config: AiraBotConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AiraBotConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
