...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("airabot doctor")}" (or "${formatCliCommand("airabot doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("airabot gateway install --force")}\` from the same --profile / AIRABOT_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.AIRABOT_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("airabot gateway install")}`),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = (service.command?.environment ?? process.env) as NodeJS.ProcessEnv;
      const unit = resolveGatewaySystemdServiceName(env.AIRABOT_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("airabot status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.airabot.ai/troubleshooting`);
}
