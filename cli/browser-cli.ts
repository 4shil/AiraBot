...
export function registerBrowserCli(program: Command) {
  const browser = program
    .command("browser")
    .description("Manage AiraBot's dedicated browser (Chrome/Chromium)")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples(
          ...
          "/cli/browser",
          "docs.airabot.ai/cli/browser",
        )}\n`,
    )
    .action(() => {
      browser.outputHelp();
      defaultRuntime.error(
        danger(`Missing subcommand. Try: "${formatCliCommand("airabot browser status")}"`),
      );
