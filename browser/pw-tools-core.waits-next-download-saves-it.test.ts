...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAiraBotTmpDir: vi.fn(() => "/tmp/airabot"),
}));
vi.mock("../infra/tmp-airabot-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAiraBotTmpDir.mockReturnValue("/tmp/airabot");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAiraBotTmpDir.mockReturnValue("/tmp/airabot-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "airabot-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAiraBotTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "airabot-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "airabot-preferred", "downloads")}${path.sep}`),
    );
  });
