...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const airabot = resolveProfile(resolved, "airabot");
    expect(airabot?.driver).toBe("airabot");
    expect(airabot?.cdpPort).toBe(18800);
    expect(airabot?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENCLAW_GATEWAY_PORT when unset", () => {
    ...
    try {
      ...
      const airabot = resolveProfile(resolved, "airabot");
      expect(airabot?.cdpPort).toBe(19012);
      expect(airabot?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const airabot = resolveProfile(resolved, "airabot");
      expect(airabot?.cdpPort).toBe(19022);
      expect(airabot?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.OPENCLAW_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "airabot");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        airabot: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("airabot");
  });
