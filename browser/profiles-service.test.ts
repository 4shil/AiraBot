...
vi.mock("./chrome.js", () => ({
  resolveAiraBotUserDataDir: vi.fn(() => "/tmp/airabot-test/airabot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAiraBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes remote profiles without stopping or removing local data", async () => {
    ...
    vi.mocked(loadConfig).mockReturnValue({
      browser: {
        ...
        profiles: {
          airabot: { cdpPort: 18800, color: "#FF4500" },
          remote: { cdpUrl: "http://10.0.0.42:9222", color: "#0066CC" },
        },
      },
    });
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "airabot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAiraBotUserDataDir).mockReturnValue(userDataDir);
