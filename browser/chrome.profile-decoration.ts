...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".airabot-profile-decorated");
}
 ...
 */
export function decorateAiraBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AIRABOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AIRABOT_BROWSER_COLOR).toUpperCase();
