...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    ...
    // Start with only airabot profile
    ...
    // 2. Simulate adding a new profile to config (like user editing airabot.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.airabot = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.airabot?.cdpPort).toBe(19999);
  });
