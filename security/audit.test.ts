...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AiraBotConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-security-audit-"));
  });
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-airabot.json");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AiraBotConfig = { session: { dmScope: "main" } };
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: AiraBotConfig = { gateway: { mode: "local" } };
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.airabot",
      configPath: "/Users/test/Dropbox/.airabot/airabot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: AiraBotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "airabot.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        airabot: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
