...
function summarizeGroupPolicy(cfg: AiraBotConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AiraBotConfig): boolean {
...
function collectModels(cfg: AiraBotConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AiraBotConfig): Set<string> {
  const baseCfg: AiraBotConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AiraBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AiraBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AiraBotConfig): boolean {
...
function isBrowserEnabled(cfg: AiraBotConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AiraBotConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AIRABOT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("airabot security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer AIRABOT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const airabotGatewayToken =
    typeof env.AIRABOT_GATEWAY_TOKEN === "string" && env.AIRABOT_GATEWAY_TOKEN.trim()
      ? env.AIRABOT_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-airabot-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AiraBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
