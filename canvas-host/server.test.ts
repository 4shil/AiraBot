...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("airabotCanvasA2UIAction");
    expect(out).toContain("airabotSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("airabotSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.airabotA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__airabot__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("airabot-a2ui-host");
      expect(html).toContain("airabotCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__airabot__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("airabotA2UI");
