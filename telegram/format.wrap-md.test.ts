...
describe("edge cases", () => {
  ...
  it("keeps .co domains as links", () => {
    const result = markdownToTelegramHtml("Visit t.co and airabot.co");
    expect(result).toContain('<a href="http://t.co">');
    expect(result).toContain('<a href="http://airabot.co">');
    expect(result).not.toContain("<code>t.co</code>");
    expect(result).not.toContain("<code>airabot.co</code>");
  });
