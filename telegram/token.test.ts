...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "airabot-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENCLAW_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "airabot-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as AiraBotConfig;
    const res = resolveTelegramToken(cfg);
