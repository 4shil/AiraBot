...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "airabot_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "airabot_cmdarg", "arg-menu options");
  });
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("airabot_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("airabot_cmdarg_ext:");
