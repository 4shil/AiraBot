...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "airabot-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: AiraBotConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/airabot" } },
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/airabot", enabled: true },
    });
