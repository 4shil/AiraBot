...
function setWhatsAppDmPolicy(cfg: AiraBotConfig, dmPolicy: DmPolicy): AiraBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AiraBotConfig, allowFrom?: string[]): AiraBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AiraBotConfig, selfChatMode: boolean): AiraBotConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: AiraBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so AiraBot can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for AiraBot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("airabot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
