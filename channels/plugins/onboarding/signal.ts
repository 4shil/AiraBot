...
function setSignalDmPolicy(cfg: AiraBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "AiraBot"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("airabot gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
