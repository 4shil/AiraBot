...
function setSlackDmPolicy(cfg: AiraBotConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "AiraBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for AiraBot`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/airabot",
          description: "Send a message to AiraBot",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: AiraBotConfig, allowFrom: string[]): AiraBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
