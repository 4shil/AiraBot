...
function telegramCfg(): AiraBotConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AiraBotConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AiraBotConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as AiraBotConfig;
