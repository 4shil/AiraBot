...
function cfg(accounts?: Record<string, unknown> | null): AiraBotConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as AiraBotConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as AiraBotConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as AiraBotConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as AiraBotConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as AiraBotConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as AiraBotConfig)).toBe("default");
    });
  });
});
