...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => AiraBotConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("airabot-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): AiraBotConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
