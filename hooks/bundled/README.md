...
This directory contains hooks that ship with AiraBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.airabot/workspace`)

**Enable**:

```bash
airabot hooks enable session-memory
```
...
```bash
airabot hooks enable bootstrap-extra-files
```
...
**Output**: `~/.airabot/logs/commands.log`

**Enable**:

```bash
airabot hooks enable command-logger
```
...
homepage: https://docs.airabot.ai/automation/hooks#my-hook
metadata:
  { "airabot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.airabot/hooks/` (shared across workspaces)
...
```bash
airabot hooks info session-memory
```
...
```bash
airabot hooks enable session-memory
airabot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.airabot/airabot.json`:
...
2. Restart gateway: `pkill -9 -f 'airabot.*gateway' && pnpm airabot gateway`
3. Enable the hook: `airabot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.airabot.ai/automation/hooks
