...
homepage: https://docs.airabot.ai/automation/hooks#command-logger
metadata:
  {
    "airabot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.airabot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.airabot/logs/commands.log ~/.airabot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/airabot`:
   ```
   /home/username/.airabot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.airabot/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.airabot/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.airabot/logs/commands.log | jq .
```
