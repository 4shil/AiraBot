...
`;
    ...
    expect(parsed.airabot.emoji).toBe("💾");
    expect(parsed.airabot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.airabot.emoji).toBe("📝");
    expect(parsed.airabot.events).toEqual(["command"]);
    expect(parsed.airabot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.airabot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"airabot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"airabot": {"events": ["test"]}}');
  });
...
describe("resolveAiraBotMetadata", () => {
  it("extracts airabot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAiraBotMetadata(frontmatter);
  ...
  it("returns undefined when airabot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        airabot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AiraBot" },
            { id: "npm", kind: "npm", package: "@airabot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@airabot/hook");
  });
...
homepage: https://docs.airabot.ai/automation/hooks#session-memory
metadata:
  {
    "airabot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with AiraBot" }],
      },
  }
---
...
`;
    ...
    const airabot = resolveAiraBotMetadata(frontmatter);
    expect(airabot).toBeDefined();
    expect(airabot?.emoji).toBe("💾");
    expect(airabot?.events).toEqual(["command:new"]);
    expect(airabot?.requires?.config).toEqual(["workspace.dir"]);
    expect(airabot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const airabot = resolveAiraBotMetadata(frontmatter);
    expect(airabot?.emoji).toBe("disk");
    expect(airabot?.events).toEqual(["command:new"]);
  });
});
