...
async function ensureAiraBotHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing airabot.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json airabot.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureAiraBotHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("airabot-hook-", async (tmpDir) => {
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await withTempDir("airabot-hook-pack-", async (tmpDir) => {
