...
const fixtureRoot = path.join(os.tmpdir(), `airabot-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@airabot/test-hooks",
        version: "0.0.1",
        airabot: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"airabot":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@airabot/test-hooks@0.0.1", "--ignore-scripts"]);
