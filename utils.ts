...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".airabot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AIRABOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AIRABOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AIRABOT_STATE_DIR.
