...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.AIRABOT_STATE_DIR;
  previousConfigPath = process.env.AIRABOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.AIRABOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.AIRABOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.AIRABOT_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.AIRABOT_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.AIRABOT_SKIP_CHANNELS;
  previousSkipProviders = process.env.AIRABOT_SKIP_PROVIDERS;
  previousSkipCron = process.env.AIRABOT_SKIP_CRON;
  previousMinimalGateway = process.env.AIRABOT_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AIRABOT_STATE_DIR = path.join(tempHome, ".airabot");
  delete process.env.AIRABOT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.AIRABOT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.AIRABOT_SKIP_GMAIL_WATCHER = "1";
  process.env.AIRABOT_SKIP_CANVAS_HOST = "1";
  process.env.AIRABOT_SKIP_CHANNELS = "1";
  process.env.AIRABOT_SKIP_PROVIDERS = "1";
  process.env.AIRABOT_SKIP_CRON = "1";
  process.env.AIRABOT_TEST_MINIMAL_GATEWAY = "1";
  process.env.AIRABOT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "airabot-test-no-bundled-extensions")
    : "airabot-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "airabot-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".airabot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      delete process.env.AIRABOT_STATE_DIR;
    } else {
      process.env.AIRABOT_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.AIRABOT_CONFIG_PATH;
    } else {
      process.env.AIRABOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.AIRABOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.AIRABOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.AIRABOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AIRABOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.AIRABOT_SKIP_CANVAS_HOST;
    } else {
      process.env.AIRABOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.AIRABOT_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.AIRABOT_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.AIRABOT_SKIP_CHANNELS;
    } else {
      process.env.AIRABOT_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.AIRABOT_SKIP_PROVIDERS;
    } else {
      process.env.AIRABOT_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.AIRABOT_SKIP_CRON;
    } else {
      process.env.AIRABOT_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.AIRABOT_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.AIRABOT_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["AIRABOT_GATEWAY_TOKEN"]);
  const prev = process.env.AIRABOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    delete process.env.AIRABOT_GATEWAY_TOKEN;
  } else {
    process.env.AIRABOT_GATEWAY_TOKEN = fallbackToken;
  }
