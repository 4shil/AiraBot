...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "airabot");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.airabot-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".airabot");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "airabot.json");
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-wizard-home-"));
    process.env.HOME = tempHome;
