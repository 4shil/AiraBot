...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("airabot-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __airabot?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__airabot?.kind).toBe("compaction");
    expect(marker.__airabot?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("airabot-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses OPENCLAW_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/airabot-home"), ".airabot", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile when storePath agent context differs", () => {
    const storePath = "/tmp/airabot/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/airabot/agents/ops/sessions/sess-safe.jsonl";
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/airabot/agents/main/sessions/sessions.json",
    );
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("airabot-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
