...
import type { AiraBotConfig } from "../../config/types.airabot.js";
import {
...
import { loadAiraBotPlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: AiraBotConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadAiraBotPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadAiraBotPlugins() already cache
