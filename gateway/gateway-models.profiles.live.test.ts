...
import { resolveAiraBotAgentDir } from "../agents/agent-paths.js";
...
import { ensureAiraBotModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, AiraBotConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AiraBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveAiraBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.airabot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAiraBotAgentDir();
  const sanitizedCfg: AiraBotConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-live-"));
  const tempConfigPath = path.join(tempDir, "airabot.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.OPENCLAW_CONFIG_PATH = tempConfigPath;

  await ensureAiraBotModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "AiraBot live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAiraBotModelsJson(cfg);
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.airabot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
