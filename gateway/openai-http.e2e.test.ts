    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "airabot", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-airabot-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-airabot-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "airabot", messages: [{ role: "user", content: "hi" }] },
          {
            "x-airabot-agent-id": "beta",
            "x-airabot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
