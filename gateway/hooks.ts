...
export function resolveHooksConfig(cfg: AiraBotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AiraBotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-airabot-token"] === "string"
      ? req.headers["x-airabot-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
