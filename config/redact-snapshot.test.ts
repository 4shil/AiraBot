...
import type { ConfigFileSnapshot } from "./types.airabot.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.airabot/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/airabot/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/airabot/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/airabot/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/airabot/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AiraBotSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AiraBotConfig";
    const hints = mapSensitivePaths(AiraBotSchema, "", {});
