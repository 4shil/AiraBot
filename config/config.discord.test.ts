...
describe("config discord", () => {
  ...
  it("loads discord guild map + dm group settings", async () => {
    await withTempHome(async (home) => {
      ...
      expect(cfg.channels?.discord?.dm?.groupChannels).toEqual(["airabot-dm"]);
      ...
      expect(cfg.channels?.discord?.guilds?.["123"]?.slug).toBe("friends-of-airabot");
