...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = AiraBotSchema.safeParse({
      $schema: "https://airabot.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://airabot.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AiraBotSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AiraBotSchema.safeParse({
      cron: {
        enabled: true,
