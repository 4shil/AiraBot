...
import type { AiraBotConfig, ConfigValidationIssue } from "./types.js";
import { AiraBotSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: AiraBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AiraBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AiraBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AiraBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AiraBotConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as AiraBotConfig,
  };
}
