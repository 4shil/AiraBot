...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("airabot-triggers-", async (home) => {
      const inboundDir = join(home, ".airabot", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "airabot"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("airabot-triggers-bypass-", async (home) => {
      // Sensitive host file outside .airabot
