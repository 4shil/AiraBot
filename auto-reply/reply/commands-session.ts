...
import { scheduleGatewaySigusr1Restart, triggerAiraBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AiraBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAiraBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AiraBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
