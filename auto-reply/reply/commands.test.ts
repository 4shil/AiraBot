...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-commands-"));
...
function buildParams(commandBody: string, cfg: AiraBotConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/airabot-session-store.json" },
    } as AiraBotConfig;
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes Slack DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    const written = writeConfigFileMock.mock.calls[0]?.[0] as AiraBotConfig;
