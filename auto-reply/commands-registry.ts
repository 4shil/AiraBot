...
export function isCommandEnabled(cfg: AiraBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AiraBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AiraBotConfig),
...
export function getCommandDetection(_cfg?: AiraBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AiraBotConfig) {
