...
type AgentDefaults = NonNullable<NonNullable<AiraBotConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.airabot/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 AiraBot ${VERSION}${commit ? ` (${commit})` : ""}`;
  ...
  return [
    ...
    `📚 ${contextLine}`,
    mediaLine,
    // args.usageLine, // PATCHED: metrics removed
    `🧵 ${sessionLine}`,
    args.subagentsLine,
    `⚙️ ${optionsLine}`,
    voiceLine,
...
export function buildHelpMessage(cfg?: AiraBotConfig): string {
  const lines = ["ℹ️ Help", ""];
