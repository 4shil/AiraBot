...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-media-test-"));
...
describe("local media root guard", () => {
  ...
  it("allows default AiraBot state workspace and sandbox roots", async () => {
  ...
  it("rejects default AiraBot state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
