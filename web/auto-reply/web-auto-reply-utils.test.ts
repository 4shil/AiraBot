...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bairabot\b/i] };

  it("ignores regex matches when other mentions are present", () => {
    const msg = makeMsg({
      body: "@AiraBot please help",
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\bairabot\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("resolves mentionedJids via lid reverse mapping in authDir", async () => {
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-lid-mapping-"));
    try {
      await fs.writeFile(
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "airabot-snapshot-"));
